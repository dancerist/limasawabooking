...
| `limasawa-rates.php` | Flexible pricing: registers a **local ACF field group** (`group_limasawa_pricing`) on `accommodation` with `pricing_mode` (flat/rooms/per_pax) + `accommodation_room_types` repeater (`room_name`/`room_capacity`/`room_rate`/`room_note`) + `additional_bed_rate` + per-pax fields (`per_pax_base_rate`/`per_pax_base_pax`/`per_pax_extra_rate`/`per_pax_max_pax`), all conditional on mode. Read via `sb_rate_summary()` in listings.php (computes `priceFrom`=cheapest nightly → card `price` + drives price filter + card "from ₱X"); written by submit.php. Defaults `flat` so the 25 existing listings are unchanged. Live & verified 2026-06-01. |
...
- **Flexible pricing LIVE** (2026-06-01): per-listing `pricing_mode` = flat (existing) | rooms (room-types repeater + additional-bed) | per_pax (base+extra/pax tiered). Backend `limasawa-rates.php` (ACF fields) + `sb_rate_summary()` in listings.php (computes `priceFrom`); submit form has a pricing-mode chooser → conditional rate sub-steps (`list-your-property.astro`); cards show "from ₱X" (`card-template.js`, threaded via `api.ts` `priceMode`); single page renders a room-rates table / per-pax block (`stay/[slug].astro` `renderRates`). Verified rooms→₱1,999-from & per_pax→₱3,400 round-trip over PHP. NOTE: edit-mode (`/update-listing/{id}`) backend endpoint does NOT exist — only create (`/submit-listing`) writes pricing; building edit is a separate task.
